...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.model_var_type = model_var_type                # ModelVarType.LEARNED_RANGE
        self.loss_type = loss_type                          # LossType.MSE
        self.snr = snr
        self.return_startx = return_startx                  # False
        self.prob_self_condition = prob_self_condition
        assert self.prob_self_condition >= 0 and self.prob_self_condition <= 1
        
        # Use float64 for accuracy.
        betas = np.array(betas, dtype=np.float64)           # (1000,)
        self.betas = betas
        ...
        assert self.alphas_cumprod_prev.shape == (self.num_timesteps,)                      # (1000,)
    ...
    def _predict_xstart_from_v(self, x_t, t, v):
        #TODO: add by self, need to check
        ...
        for i in indices:
            ...
            if False:
                with th.no_grad():
                    ...
                    yield out
                    img = out["sample"]
                    if self.prob_self_condition > 0:
                        model_kwargs['x_self_cond'] = out["pred_xstart"]
            else:
                with th.no_grad():
                    if self.prob_self_condition > 0:
                        model_kwargs['x_self_cond'] = None
                        x_self_cond = self.p_mean_variance(
                            model, img, t, 
                            clip_denoised=True,
                            denoised_fn=None,
                            model_kwargs=model_kwargs
                        )['pred_xstart']
                        model_kwargs['x_self_cond'] = x_self_cond.detach()
    ...
    def training_losses(self, model, x_start, timestep, model_kwargs=None, noise=None, skip_noise=False):
        ...
        if skip_noise:
            ...
            if noise is None:
                noise = th.randn_like(x_start)                  # [B, C, T, H, W]
            #TODO: add noise offset: https://www.crosslabs.org/blog/diffusion-with-offset-noise
            
            x_t = self.q_sample(x_start, t, noise=noise)
        
        if random.uniform(0, 1) < self.prob_self_condition:
            with th.no_grad():
                if False:
                    #TODO: self-cond
                    x_t_1 = self.q_sample(x_start, (t + 1).clamp(max=self.num_timesteps), noise=noise)
                    x_self_cond = self.p_mean_variance(
                        model, 
                        x_t_1, 
                        t, 
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if isinstance(model_output, dict) and model_output.get('x', None) is not None:
                ...
                output = model_output                       # [B, C*2, T, H, W]
            ...
            target = {
                ...
                ModelMeanType.V: _extract_into_tensor(self.sqrt_alphas_cumprod, t, x_t.shape) * noise - _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape) * x_start
